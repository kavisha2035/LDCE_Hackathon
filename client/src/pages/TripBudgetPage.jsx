...
import {
  ...
  Ticket, CheckCircle2, ChevronRight, Sliders, Info
} from 'lucide-react';

export default function TripBudgetPage({ tripId, onBack, onNavigateToCalendar }) {
  ...
  // User-configurable budget threshold (per day or total)
  const [budgetLimit, setBudgetLimit] = useState(2000);
  const [perStopLimit, setPerStopLimit] = useState(900);
  const [showLimitConfig, setShowLimitConfig] = useState(false);

  // Active chart tab (overview, by-stop, by-category)
  const [activeView, setActiveView] = useState('all'); // 'all', 'stops', 'categories'

  // Fetch all trips for switcher if no tripId or to allow switching
  useEffect(() => {
    fetch('/api/trips')
      .then(res => res.json())
      .then(data => {
  ...
  const fetchBudget = () => {
    ...
    fetch(`/api/trips/${selectedTripId}/budget`)
      .then(res => {
        if (!res.ok) throw new Error(`HTTP ${res.status}: Failed to fetch budget`);
        return res.json();
      })
      .then(data => {
        if (data.budget) {
  ...
  const budgetDifference = total - budgetLimit;
  ...
  // Max cost among stops for normalized bar chart
  const maxStopCost = budgetData?.stops?.reduce((max, s) => Math.max(max, s.stopTotal), 1) || 1;
  ...
  return (
    <div className="space-y-8 font-body">
      ...
      {!loading && !error && budgetData && (
        <>
          {/* Over-Budget Alert Banner — Stamp-Red (#B84A3E) Reserved Exclusively for Alerts */}
          {isOverBudget ? (
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="font-display font-bold text-lg uppercase tracking-wide">
                    OVER-BUDGET ALERT: EXCEEDED BY ${budgetDifference.toFixed(2)}
                  </div>
                  <div className="font-mono text-xs text-white/90">
                    Total expenses (${total.toFixed(2)}) exceed your target ceiling of ${budgetLimit.toFixed(2)}.
                  </div>
                </div>
              </div>
              <button
                onClick={() => setShowLimitConfig(!showLimitConfig)}
                className="px-3 py-1.5 bg-white text-[#B84A3E] font-mono text-xs font-bold uppercase border border-[#1F2B2E] hover:bg-[#F6F3EC]"
              >
                ADJUST CEILING
              </button>
            </div>
            ...
            <div className="bg-white border-2 border-[#1F2B2E] p-3 shadow-[3px_3px_0px_0px_#1F2B2E] flex items-center justify-between flex-wrap gap-3">
              <div className="flex items-center gap-2">
                ...
                <span className="font-mono text-xs text-[#1F2B2E] font-bold uppercase">
                  BUDGET HEALTHY: ${(budgetLimit - total).toFixed(2)} REMAINING UNDER ${budgetLimit} CEILING ({percentUsed}% COMMITTED)
                </span>
              </div>
              <button
                onClick={() => setShowLimitConfig(!showLimitConfig)}
                className="font-mono text-[11px] text-[#2C5F7C] font-bold uppercase underline hover:text-[#1F2B2E]"
              >
            ...
            <div className="bg-white border-2 border-[#1F2B2E] p-6 shadow-[4px_4px_0px_0px_#1F2B2E] flex flex-col justify-between">
              <div>
                ...
                <div className="text-4xl sm:text-5xl font-extrabold font-display tracking-tight text-[#1F2B2E]">
                  ${budgetData.tripTotal.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                </div>
              </div>
            ...
            <div className="bg-white border-2 border-[#1F2B2E] p-6 shadow-[4px_4px_0px_0px_#1F2B2E] flex flex-col justify-between">
              <div>
                ...
                <div className="text-4xl sm:text-5xl font-extrabold font-display tracking-tight text-[#B8823A]">
                  ${budgetData.avgPerDay.toFixed(2)}
            ...
            <div className="bg-white border-2 border-[#1F2B2E] p-6 shadow-[4px_4px_0px_0px_#1F2B2E] flex flex-col justify-between">
              ...
              <div className="space-y-2 font-mono text-xs">
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-bold text-[#1F2B2E]">${stayCost.toFixed(2)} ({stayPct}%)</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="flex items-center gap-1.5 text-[#1F2B2E]">
                  ...
                  <span className="font-bold text-[#1F2B2E]">${transportCost.toFixed(2)} ({transportPct}%)</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="flex items-center gap-1.5 text-[#1F2B2E]">
                  ...
                  <span className="font-bold text-[#1F2B2E]">${activitiesCost.toFixed(2)} ({activitiesPct}%)</span>
                </div>
              </div>
          ...
          {/* Section 2: Visual Charts (Bar Chart per Stop & Donut/Breakdown) */}
            ...
            <div className="lg:col-span-2 bg-white border-2 border-[#1F2B2E] p-6 shadow-[4px_4px_0px_0px_#1F2B2E] space-y-5">
              ...
              <div className="space-y-4 pt-2">
                {budgetData.stops?.map((stop, index) => {
                  const percentOfMax = Math.round((stop.stopTotal / maxStopCost) * 100);
                  const isStopOverLimit = perStopLimit > 0 && stop.stopTotal > perStopLimit;

                  return (
                    <div key={stop.stopId} className="space-y-1.5">
                      ...
                      <div className="w-full bg-[#F6F3EC] h-6 border border-[#1F2B2E] flex overflow-hidden relative">
                        ...
                        <div
                          ...
                          title={`Stay: $${stop.stayCost}`}
                        >
                          {stop.stayCost > 100 && `$${stop.stayCost}`}
                        </div>
                        {/* Transport Segment */}
                        <div
                          ...
                          title={`Transport: $${stop.transportCost}`}
                        >
                          {stop.transportCost > 80 && `$${stop.transportCost}`}
                        </div>
                        {/* Activities Segment */}
                        <div
                          ...
                          title={`Activities: $${stop.activitiesCost}`}
                        >
                          {stop.activitiesCost > 50 && `$${stop.activitiesCost}`}
                        </div>
                      </div>
                    </div>
                  );
            ...
            <div className="bg-white border-2 border-[#1F2B2E] p-6 shadow-[4px_4px_0px_0px_#1F2B2E] flex flex-col justify-between space-y-4">
              ...
              <div className="space-y-2 font-mono text-xs">
                <div className="p-2.5 bg-[#2C5F7C]/10 border border-[#2C5F7C]/40 flex items-center justify-between">
                  ...
                  <span className="font-bold text-[#2C5F7C]">${stayCost.toFixed(2)}</span>
                </div>
                ...
                <div className="p-2.5 bg-[#B8823A]/10 border border-[#B8823A]/40 flex items-center justify-between">
                  ...
                  <span className="font-bold text-[#B8823A]">${transportCost.toFixed(2)}</span>
                </div>
                ...
                <div className="p-2.5 bg-[#7FA69C]/10 border border-[#7FA69C]/40 flex items-center justify-between">
                  ...
                  <span className="font-bold text-[#7FA69C]">${activitiesCost.toFixed(2)}</span>
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-[#1F2B2E]/20 pb-3">
              ...
              {onNavigateToCalendar && (
                <button
                  ...
                  className="font-mono text-xs font-bold text-[#2C5F7C] uppercase flex items-center gap-1 hover:underline"
                >
              ...
              <table className="w-full text-left font-mono text-xs border-collapse">
                ...
                <tfoot>
                  <tr className="bg-[#1F2B2E] text-[#F6F3EC] font-bold text-sm">
                    ...
                    <td className="p-3 text-right">${stayCost.toFixed(2)}</td>
                    <td className="p-3 text-right">${transportCost.toFixed(2)}</td>
                    <td className="p-3 text-right">${activitiesCost.toFixed(2)}</td>
